...
export const LAZY_IN_SYNC = (
	key: unknown,
): string => `You are attempting to construct '${key}' in a synchronous way
...
export const ARGUMENTS_LENGTH_MISMATCH = (...values: unknown[]): string =>
...
export const POST_CONSTRUCT_ERROR = (
	clazz: string,
	errorMessage: string,
): string => `@postConstruct error in class ${clazz}: ${errorMessage}`;
