...
test('Err<E> | Ok<T> should be Result<T, E>', () => {
	...
	expect(Result.isResult(new Some(3))).toEqual(false);
...
test('Result.all', () => {
	...
	expect(all5).toMatchResult(new Err(9));
...
test('Result.wrap', () => {
	...
	expect(b).toMatchResult(new Err(err));
...
test('Result.wrapAsync', async () => {
	...
	expect(c).toMatchResult(new Err('thrown before promise'));
...
test('Issue #24', () => {
	const getStatus = (payload: boolean): Result<boolean, Error> => {
		...
		return new Err(new Error('Payload is false'));
	};

	const processStatus = (): Result<boolean, Error> => {
		return getStatus(true)
			.andThen((result) => new Ok(result))
			.map((data) => data);
	};
});
...
test('To option', () => {
	...
	expect(option).toEqual(new Some('hello'));
